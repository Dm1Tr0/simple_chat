...
int interactions (int servid)
{
    ...
    if((f = open(FILEPATH, O_RDWR | O_CREAT | O_TRUNC | O_NONBLOCK, S_IRWXU | S_IRWXG)) < 1) {
    ...
    if(pthread_create(&read_thread_id, NULL, get_the_messeges,(void*)f_s)) {
        log("error whilwe opening file");
    }
        printf("enter : /help and /in_msg to get help msg from server \n");
